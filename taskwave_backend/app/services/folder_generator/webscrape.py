...
SINGLE_TIME_RE = re.compile(r'(\d{1,2})[:\.]?([0-5]\d)')
ROOM_RE = re.compile(
...
def _html_to_text(html: str) -> str:
    ...
    s = re.sub(r'(?i)<\s*(tr|li|p|div|h\d|th|td)[^>]*>', '\n', s)
    s = re.sub(r'<[^>]+>', ' ', s)
...
if __name__ == "__main__":
    import sys, json
    if len(sys.argv) < 2:
        print("Usage: python webscrape.py <url> [--mode=auto|kor|dom|text|css] [--diagnose]")
        sys.exit(1)
    url = sys.argv[1]
    mode = "auto"
    diagnose = False
    for arg in sys.argv[2:]:
        ...
        if arg == "--diagnose":
            diagnose = True
    if diagnose:
        print(json.dumps(diagnose_schedule_page(url), indent=2, ensure_ascii=False))
    else:
        res = parse_schedule_from_web(url, mode=mode)
